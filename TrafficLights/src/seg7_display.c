...
void calculateDigits(s16 number)
{
	...
	if (number >=0) { // Check if non-negative
		...
		if (number > 999)
		{
			...
			fourthDigit  = abs(number) % 10;
			thirdDigit = (abs(number) / 10) % 10;
			secondDigit  = (abs(number) / 100) % 10;
			firstDigit = abs(number) / 1000;
		}
	...
	else if(number < 0) { // Then number must be negative
